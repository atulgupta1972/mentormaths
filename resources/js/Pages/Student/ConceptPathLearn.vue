...
const index = ref(Math.min(
    Number(props.progress?.current_card_index ?? 0),
    Math.max(0, (props.path?.cards?.length || 1) - 1),
));
const finished = ref(props.progress?.status === 'completed');
...
const recordQuiet = (payload) => {
    if (!props.routes?.record_card) {
        return;
    }
    router.post(props.routes.record_card, payload, {
        preserveScroll: true,
        preserveState: true,
        only: ['progress'],
    });
...
const markComplete = () => {
    finished.value = true;
    if (!props.routes?.complete) {
        return;
    }
    router.post(props.routes.complete, {}, {
        preserveScroll: true,
        preserveState: true,
        only: ['progress'],
    });
...
const goNext = () => {
    ...
    const leavingIndex = index.value;
    const leaving = current.value;
    const hadCheck = isCheck.value || isAngleMap.value || isTurnClock.value;

    recordQuiet({
        card_index: leavingIndex,
        card_step: leaving?.step ?? leavingIndex + 1,
        card_type: leaving?.type ?? null,
        correct: hadCheck ? isCurrentCorrect.value : null,
...
<template>
    <Head :title="`Learn concepts · ${chapter.label}`" />
        ...
        <div class="py-8">
            <div class="mx-auto max-w-2xl space-y-4 px-4 sm:px-6">
                ...
                <div v-if="finished" class="rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-center shadow-sm">
                    <p class="text-lg font-semibold text-emerald-950">Concepts complete</p>
                    <p class="mt-1 text-sm text-emerald-900">
                        You finished {{ total }} concept cards for {{ chapter.label }}.
                    </p>
                    <div class="mt-4 flex flex-wrap justify-center gap-2">
                        <PrimaryButton type="button" @click="restart">Learn again</PrimaryButton>
                        <Link :href="chapter.study_plan_url">
                            <SecondaryButton type="button">Back to study plan</SecondaryButton>
                        </Link>
                    </div>
                </div>
                ...
                >
                    ...
                    <template v-else-if="isTurnClock">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <p class="text-base font-semibold text-slate-900">
                                {{ currentPrompt?.prompt || 'Rotate the hand to make the turn.' }}
                            </p>
