...
const selectOption = (optionId) => {
    if (submitting.value || feedback.value?.correct || feedback.value?.failed) {
        return;
    }
...
const optionClass = (optionId) => {
    ...
    if (feedback.value?.failed && optionId === feedback.value.correct_option_id) {
        return 'border-green-500 bg-green-50 ring-2 ring-green-300';
    }

    if (feedback.value?.failed && optionId === selectedOptionId.value) {
        ...
        <template #header>
            <div>
                ...
                <p class="text-sm text-gray-500">
                    One attempt per question — wrong answers come back at the end
                ...
                <div v-if="question" class="overflow-hidden rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                    ...
                    <div class="mt-5 space-y-2">
                        <button
                            ...
                            :disabled="submitting || feedback?.correct || feedback?.failed"
                    ...
                    >
                        <p class="font-medium">Not correct — we&apos;ll ask again at the end of today&apos;s drill.</p>
                    ...
                    <div class="mt-6 flex items-center justify-end">
                        <PrimaryButton
                            type="button"
                            :disabled="!selectedOptionId || submitting || feedback?.correct || feedback?.failed"
                            @click="submitAnswer"
                        >
