...
const canSubmit = computed(() => canEdit.value
    ...
    && ! (props.progress.missing_diagram > 0));

const nonNumericRows = computed(() =>
    props.rows.filter((row) => !row.skipped && row.non_numeric_answer),
);

const missingDiagramRows = computed(() =>
    props.rows.filter((row) => !row.skipped && row.missing_diagram),
);
...
const deleteMissingDiagramRow = (row) => {
    if (!window.confirm('Remove this question from fill-in-blank conversion? It needs a figure that is missing. The MCQ stays.')) {
        return;
    }
...
const deleteAllMissingDiagrams = () => {
    if (!missingDiagramRows.value.length) {
        return;
    }

    if (!window.confirm(`Remove ${missingDiagramRows.value.length} question(s) that need a figure but have none? MCQs stay.`)) {
        return;
    }

    router.post(route('content.tasks.convert-clear-missing-diagrams', props.task.id), {}, {
        preserveScroll: true,
        only: ['rows', 'progress', 'flash', 'task'],
    });
};
        ...
        <div class="py-8">
            <div class="mx-auto max-w-4xl space-y-4 px-4 sm:px-6">
                ...
                <div class="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-700">
                    ...
                    Questions that need a figure but have none must be removed from conversion (or upload the figure on the MCQ chapter first) — fill-in-blank practice must show the same picture.
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <p class="text-sm text-slate-600">
                        ...
                        <span v-if="progress.missing_diagram" class="text-rose-700">
                            · {{ progress.missing_diagram }} missing figure
                        </span>
                    </p>
                    <div class="flex flex-wrap gap-2">
                        <button
                            v-if="canEdit && missingDiagramRows.length"
                            type="button"
                            class="rounded-md border border-rose-300 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-900 hover:bg-rose-100"
                            @click="deleteAllMissingDiagrams"
                        >
                            Remove all missing figures ({{ missingDiagramRows.length }})
                        </button>
                        <button
                ...
                >
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <span
                                v-else-if="row.missing_diagram"
                                class="rounded-full bg-rose-100 px-2 py-0.5 text-[11px] font-semibold text-rose-950"
                            >Missing figure</span>
                            <span
                    ...
                    <div class="mt-3 rounded-md bg-slate-50 p-3 text-sm">
                        ...
                        <div v-if="row.diagram_preview_url" class="mt-2">
                            <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Figure (must appear on fill-in-blank too)</p>
                            <img
                                :src="row.diagram_preview_url"
                                alt="Question figure"
                                class="mt-1 max-h-40 max-w-full rounded border border-slate-200 object-contain"
                            >
                        </div>
                        <p
                            v-else-if="row.needs_diagram"
                            class="mt-2 rounded bg-rose-100 px-2 py-1 text-[11px] font-semibold text-rose-950"
                        >
                            This sum needs a figure — upload it on the MCQ chapter, or remove from conversion.
                        </p>
                    ...
                    <template v-if="!row.skipped">
                        <div
                            v-if="row.missing_diagram"
                            class="mt-3 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-xs text-rose-950"
                        >
                            Fill-in-blank cannot ship without the figure. Remove this row from conversion, or upload the diagram on the textbook chapter first.
                        </div>
                        <div
